// src/App.tsx
import { stopConnection } from "./lib/signalrClient";
...
import { getUserByUserName, type UserDto } from "./api/users";
...
function App() {
  ...
  // aktuális user ref-ben, hogy a handlerek mindig a legfrissebbet lássák
  const currentUserRef = useRef<CurrentUser | null>(null);
  useEffect(() => {
    currentUserRef.current = currentUser;
  }, [currentUser]);

  // hogy a SignalR handlereket csak egyszer regisztráljuk
  ...
  async function initForUser(user: CurrentUser): Promise<void> {
    ...
    try {
      ...
      // SignalR event handlerek – CSAK EGYSZER regisztráljuk
      if (!handlersRegisteredRef.current) {
        ...
        onUserTyping((ev: TypingEvent) => {
          ...
          if (ev.userId.toLowerCase() === current.id.toLowerCase()) {
            return;
          }
      ...
      // belépés a default szobába + history betöltése
  ...
  const handleLogout = async () => {
    // 1) SignalR leállítása
    await stopConnection();

    // 2) localStorage törlése
    localStorage.removeItem(LOCAL_STORAGE_USER_KEY);
              ...
              >
                {messages.map((m) => (
                  <div key={m.id} style={{ marginBottom: "10px" }}>
                    {/* Ki küldte (saját üzenetnél elhagyhatjuk vagy megjeleníthetjük más színnel) */}
                    {!m.isOwn && (
                      <div
                        style={{
                          fontSize: "12px",
